...
describe('parseAcceptLanguage', () => {
	...
	it('orders locales by descending quality', () => {
		expect(parseAcceptLanguage('fr;q=0.5,en;q=0.9,de;q=0.7')).toEqual([
			'en',
			'de',
			'fr',
		]);
	...
	it('treats missing q as 1.0', () => {
		expect(parseAcceptLanguage('fr;q=0.5,en,de;q=0.7')).toEqual([
			'en',
			'de',
			'fr',
		]);
	...
	it('preserves input order for ties (stable sort)', () => {
		expect(parseAcceptLanguage('en;q=0.8,fr;q=0.8,de;q=0.8')).toEqual([
			'en',
			'fr',
			'de',
		]);
	...
	it('trims surrounding whitespace around locales and parameters', () => {
		expect(parseAcceptLanguage('  en-US , fr ; q=0.8 , de;q=0.5')).toEqual([
			'en-US',
			'fr',
			'de',
		]);
	...
	it('ignores unknown parameters and keeps q', () => {
		expect(parseAcceptLanguage('en;foo=bar;q=0.4,fr;q=0.7')).toEqual([
			'fr',
			'en',
		]);
	});
